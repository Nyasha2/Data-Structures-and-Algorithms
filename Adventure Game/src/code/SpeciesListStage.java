...
public class SpeciesListStage implements AdventureStage {
    ...
    public String riddlePrompt() {
        return "Wow! That was pretty neat! We got to see so many neat animals! Would you like to get a drink of water at Caltech Hall? Or maybe some food at Browne?";
    }
    ...
    private void handleResponses(List<String> reference) {
        while (true) {
            ...
            List<String> user = new ArrayList<>(Arrays.asList(input.toLowerCase().split(",")));
            int size = user.size();
            for (int i = 0; i < user.size(); i++) {
                if (user.get(i).trim().isEmpty()) {
                    user.remove(i);
                }
            }
